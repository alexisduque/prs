...
/***
   ...
   ajour des adresses pour les �tudiants
   ...
   quelques modification
    - ajout d'un champs listening_ipn dans l'�tat du server
...
//cach�e au sien du module p2p_addr.c
typedef struct p2p_addr_struct *p2p_addr;

//Cr�er et donc alloue une structure p2p_addr. retourne une p2p_addr,
//donc un pointeur sur la structure cr�e
p2p_addr p2p_addr_create();

//Supprime une adresse p2p_addr. (i.e., supprime la structure point�
//par addr
void     p2p_addr_delete(p2p_addr addr);

//Copie l'adresse source src dans l'adress destination dst. La
//structure dst doit �tre allou�e avant.
...
//Compare les 2 adresses P2P. Renvoie 0 si elle sont diff�rentes,
//i.e., si la partie IP n'est pas la m�me ou si les ports employ�s
//sont diff�rents.
...
//Renvoie 0 si l'adresse addr n'est pas l'adresse non d�finie P2P
...
//renvoie un pointeur sur l'adresse P2P non d�finie
p2p_addr p2p_addr_undefined();

//assigne l'adresse P2P dst. Les param�tres sont l'adresse IP ip_str sous
...
//assigne l'adresse dst � partir d'une adresse sous forme de string,
//i.e., IP:TCP_PORT:UDP_PORT
int   p2p_addr_setstr(p2p_addr dst, const char* p2p_str);

//assigne l'adresse addr � P2P broadcast : 255.255.255.255:0:0
void  p2p_addr_set_broadcast(p2p_addr addr);

//assigne l'adresse addr � P2P undifinied : 0.0.0.0:0:0
...
//ecrit l'adresse format�e addr dans le fichier fd 
